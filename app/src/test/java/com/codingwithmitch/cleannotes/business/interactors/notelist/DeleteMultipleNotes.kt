...
class DeleteMultipleNotesTestUseCase : BaseUseCaseToolsTest() {
    ...
    fun deleteNotes_success_confirmNetworkAndCacheUpdated() = runBlocking {
        ...
        println("notes = ${notesInCache.size}")
        ...
        val doNotesExistInNetwork = noteRemoteDataSource.getAllNotes()
            .containsAll(randomNotes)
        assertFalse { doNotesExistInNetwork }


        // confirm notes are added to "deletes" node in network
        val deletedNetworkNotes = noteRemoteDataSource.getDeletedNotes()
